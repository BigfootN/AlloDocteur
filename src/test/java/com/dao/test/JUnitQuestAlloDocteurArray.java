...
public class JUnitQuestAlloDocteurArray {
	...
	public void testFindAllUtilisateurs() {
		...
		log.debug("Entree de la methode");
		Assert.assertTrue(
			users.findAllUtilisateurs().size() ==
			DaoHelper.getUtilisateursArrayDataSource().length);
		log.debug("Sortie de la methode");
	}
	...
	public void testFindAllAdresses() {
		...
		log.debug("Entree de la methode");
		Assert.assertTrue(
			addresses.findAllAdresses().size() == DaoHelper.getAdressesArrayDataSource().length);
		log.debug("Sortie de la methode");
	}
	...
	public void testCreateUpdateDeleteUtilisateur() {
		...
		Assert.assertTrue(
			listUser.findAllUtilisateurs().size() ==
			(DaoHelper.getUtilisateursArrayDataSource().length + 1));
	...
	public void testCreateUpdateDeleteAdresse() {
		...
		Assert.assertTrue(listUser.findAllAdresses().size() ==
						  (DaoHelper.getAdressesArrayDataSource().length + 1));
